...
/*  if (!file_revert)
    return file_bufferstart + file_ptr;
  else
    return file_bufferstart - file_ptr;*/
//  return (XLONG) ftell (file_file);
}
...
/*int file_fillBuffer ()
{
  ...
  if (bytesread != file_ptr)
  {
    if (file_bufferlen - 1 >= 0)
      file_buffer [file_bufferlen - 1] = 0;
  }
  file_ptr = 0;
  return 1;
}*/
...
/*int file_fillBuffer ()
{
  int bytesread = 0;
  int bytestoread = file_ptr;
  int filemaxlen = FILE_MAXLEN - 1;
  int n = FILE_MAXLEN - 1 - file_ptr;

  if (file_ptr <= filemaxlen / 2 && file_ptr <= file_bufferlen) return 1;
  if (file_ptr < filemaxlen && file_ptr >= 0)
    if (file_buffer [file_ptr] == 0)
      return 0;
  ...
  if (file_reverse)
  {
    n = file_bufferlen - file_ptr - 1;
    if (file_bufferstart - bytestoread >= 0)
    {
      fseek (file_file, -bytestoread - file_bufferread, SEEK_CUR);
      file_bufferstart += -bytestoread - file_bufferread;
    }
    ...
    {
      bytestoread = file_bufferstart;
      fseek (file_file, 0, SEEK_SET);
      memset (&file_buffer [n + bytestoread], 0, filemaxlen - (n + bytestoread));
    }
  }
...
/*int file_nextDouble (double *d)
{
  char *ptr;
  int readlen;
  *d = strtod (&file_buffer [file_ptr], &ptr);
  readlen = (int) (ptr - &file_buffer [file_ptr]);
  file_ptr += readlen;
