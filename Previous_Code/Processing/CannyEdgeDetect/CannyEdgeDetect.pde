...
void setup() 
{
  TesterImage=loadImage("Off.JPG");
  size(TesterImage.width, TesterImage.height);
  noLoop();
}

void draw() 
{
  image(TesterImage, 0, 0);
  CannyEdgeDetector detector = new CannyEdgeDetector();
  //adjust its parameters as desired
  detector.setLowThreshold(0.5f);
  detector.setHighThreshold(1f);
  //apply it to an image
  detector.setSourceImage(TesterImage);
  detector.process();
...
class CannyEdgeDetector {
	// statics
	private float GAUSSIAN_CUT_OFF = 0.005f;
	private float MAGNITUDE_SCALE = 100F;
	private float MAGNITUDE_LIMIT = 1000F;
	private int MAGNITUDE_MAX = (int) (MAGNITUDE_SCALE * MAGNITUDE_LIMIT);
	// fields
	private int Height;
	private int Width;
	private int picsize;
	...
	private void writeEdges() {
		edgesImage = createImage(Width, Height, ARGB);
                for(int i=0; i < picsize; i++) {
                  edgesImage.pixels[i] = color(data[i]); 
                }
	}
}
